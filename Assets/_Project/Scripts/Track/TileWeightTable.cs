...
namespace TempleSprint
{
    ...
    public class TileWeightTable : ScriptableObject
    {
        ...
        {
            float straight, coin, obstacle, gap, branch, dyn, env, river, fire, zipline, minecart, turnL, turnR, junction;
            ...
            switch (difficulty)
            {
                ...
                case RunDifficulty.Hard:
                    straight = 0.08f - difficultyTier * 0.01f;
                    coin = 0.08f;
                    obstacle = allowHazard ? 0.14f + obstacleBias * 0.28f + difficultyTier * 0.03f : 0f;
                    ...
                    env = 0.03f;
                    river = allowHazard ? 0.10f * riverBias + difficultyTier * 0.01f : 0f;
                    fire = allowHazard ? 0.09f * fireBias + difficultyTier * 0.01f : 0f;
                    zipline = allowHazard ? 0.08f * zipBias : 0f;
                    minecart = allowHazard ? 0.08f * cartBias : 0f;
                    ...
                    break;
                default: // Medium
                    straight = 0.18f - difficultyTier * 0.015f;
                    coin = 0.13f;
                    obstacle = allowHazard ? 0.10f + obstacleBias * 0.18f + difficultyTier * 0.02f : 0f;
                    gap = allowHazard ? 0.04f + difficultyTier * 0.01f : 0f;
                    branch = allowHazard ? 0.04f : 0f;
                    dyn = allowHazard ? 0.04f + difficultyTier * 0.015f : 0f;
                    env = 0.04f;
                    river = allowHazard ? 0.09f * riverBias : 0f;
                    fire = allowHazard ? 0.08f * fireBias : 0f;
            ...
            float total = Mathf.Max(0.01f, straight) + coin + obstacle + gap + branch + dyn + env
                          + river + fire + zipline + minecart + turnL + turnR + junction;
            ...
            TileKind[] kinds =
            {
                ...
                TileKind.FireCrossing, TileKind.Zipline, TileKind.MineCart,
